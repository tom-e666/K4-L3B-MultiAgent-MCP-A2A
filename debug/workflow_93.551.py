...
# (run 90.89 fetched it for every topic; kept from run 91.88).
...
# customer-history timeline (previously only order_purchase_timestamp).
# Run 91.883 (extra conflict fields) changed nothing -> back to the single conflict.
CONFLICT_FIELDS: tuple[str, ...] = ()
# Experiment: keep the policy's responsible parties verbatim instead of substituting the
# case seller (affects late_delivery_seller and unavailable_order_paid only).
# Run 91.36: consistency -5, semantic unchanged -> keep substituting the case seller.
...
# Experiment after 93.128: claim_assessments is optional in the schema and its verdicts are
# not scored; its per-claim evidence lists may be what evidence precision is computed on.
...
def _payment(payment: dict[str, Any] | None, refund: dict[str, Any] | None,
    ...
    if (verdict == "duplicate_capture" and DUPLICATE_REFUNDABLE_IS_DUPLICATE and captures
            and refundable is not None):
        first = _money(captures[0].get("amount_brl")) or Decimal(0)
        refundable = max(Decimal(0), refundable - first)
...
async def solve_case(
    ...
    if order_row and output["data_conflicts"]:
        for name in CONFLICT_FIELDS:
            if len(output["data_conflicts"]) >= 5:
                break
            if order_row.get(name) != order.get(name):
                output["data_conflicts"].append({
                    "field": name, "sources": ["order", "customer_history"],
    ...
    if (UNDELIVERED_PAST_ESTIMATE_IS_LOST and shipment["verdict"] == "insufficient_evidence"
            and order.get("order_delivered_carrier_date")
    ...
    if (decision_topic and rule and policy_evidence
            and (payment_evidence or topic in PAYMENT_SKIP_TOPICS)):
        ...
        if POLICY_PARTIES_VERBATIM:
            parties = [{"party_type": party.get("party_type"),
                        "party_id": party.get("party_id")} for party in parties]
        elif shipment["late_seller_ids"]:
            parties = [
