...
def _select_history(case: dict[str, Any], history: dict[str, Any] | None) -> tuple[
    str | None, dict[str, Any], datetime | None, datetime | None, list[str]
]:
    ...
    eligible = [(at, row) for row in rows
    ...
    start, selected = eligible[0]
    order_id = selected["order_id"]
    later = sorted(at for row in rows if row.get("order_id") == order_id
                   if (at := _time(row.get("order_purchase_timestamp"))) and at > start)
    return order_id, selected, start, later[0] if later else None, _unique(
...
def _payment(payment: dict[str, Any] | None, refund: dict[str, Any] | None,
    ...
    events = _window(_rows(data.get("events")), "event_at", start, end)
    refunds = _window(_rows(_obj(refund.get("data") if refund else None).get("events")),
                      "event_at", start, end)
...
async def solve_case(
    ...
    order_id, selected, start, end, related = _select_history(case, history)
    candidates = _unique(case.get("candidate_order_ids", []))
    ...
    items = _window(item_rows, "shipping_limit_date", start, end) or item_rows
    order_row = _obj(order_evidence.get("data") if order_evidence else None)
    if selected and order_row and selected.get("order_purchase_timestamp") != order_row.get(
    ...
    shipment_evidence = await work.fetch("get_shipment_summary", "shipment", order_id=order_id)
    shipment, shipment_ids = _shipment(order, shipment_evidence, items, start, end)
    output["shipment_analysis"] = shipment
    output["affected_entities"]["shipment_ids"] = shipment_ids
    ...
    payment, payment_refs = _payment(payment_evidence, refund_evidence, items, start, end)
    output["payment_analysis"] = payment
    output["affected_entities"]["payment_references"] = payment_refs
    ...
    if decision_topic and rule and payment_evidence and policy_evidence:
        ...
        output["assessment"] = {
            ...
            "confidence": 0.88 if not output["data_conflicts"] else 0.77,
        }
        ...
        output["assessment"] = {
            ...
            "case_status": "no_action", "confidence": 0.72,
        }
